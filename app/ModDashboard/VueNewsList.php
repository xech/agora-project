...
foreach($newsList as $tmpNews)
{
	...
	////	Détails :  Date de création & Auteur  / News à la une (haut de liste) / News archivée (offline) / Date de mise en ligne/hors ligne
	$newsDetails=null;
	...
	echo $tmpNews->divContainer($newsClass).$tmpNews->contextMenu()."
			...
			<div class='vNewsDetail'>".$newsDetails.$tmpNews->menuAttachedFiles()."</div>
		 </div>";
}