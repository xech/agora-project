...
<style>
.vEventDetails						{margin-top:15px;}
.vEventDetails img, .categoryColor	{margin-right:15px;}
.vEventDetails img					{max-width:24px;}
...
<div>
	...
	if(!empty($labelPeriod))			{echo '<hr><div class="vEventDetails"><img src="app/img/reload.png">'.$labelPeriod.'</div>';}
	...
	if(!empty($contentVisibility))		{echo '<hr><div class="vEventDetails" title="'.Txt::trad("CALENDAR_visibilityTooltip").'"><img src="app/img/displayHide.png">'.$contentVisibility.'</div>';}

	////	VISIOCONFERENCE / FICHIERS JOINTS
	if(!empty($curObj->visioUrl))		{echo '<hr><div class="vEventDetails" onclick="launchVisio(\''.$curObj->visioUrl.'\')"><img src="app/img/visioSmall.png">'.Txt::trad("VISIO_launchFromEvent").'</div>';}
