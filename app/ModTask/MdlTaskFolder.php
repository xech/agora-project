...
class MdlTaskFolder extends MdlFolder
{
	...
	public function folderDetails()
	{
		...
		if(!empty($tasks["dateMin"]) && !empty($tasks["dateMax"])){
			//// Label
			$barLabel='<img src="app/img/task/date.png"> <span class="progressBarLabel">'.Txt::dateLabel("dateMini",$tasks["dateMin"],$tasks["dateMax"]).'</span>';
			$barTooltip=Txt::trad("TASK_folderDateBegin").' : '.Txt::dateLabel("dateDefault",$tasks["dateMin"]).'<br>'.
						Txt::trad("TASK_folderDateEnd").' : '.Txt::dateLabel("dateDefault",$tasks["dateMax"]);
