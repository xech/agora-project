...
class CtrlTask extends Ctrl
{
		...
		if(!empty($timelineBegin)){
			...
			foreach(Tool::periodDays($timelineBegin,$timelineEnd) as $dateDay=>$tmpDay){									//Parcourt chaque jour de la période
				...
				$vDatas["timelineDays"][]=[
					...
					"dayLabelTitle"=>Txt::dateLabel("dateMini",$timeDayBegin)
				];
	...
	public static function getPlugins($params)
	{
		...
		foreach(MdlTask::getPluginObjects($params) as $tmpObj){
			...
			if(!empty($tmpObj->dateBegin))		{$tmpObj->pluginTooltip.="<hr>".Txt::trad("begin")." : ".Txt::dateLabel("dateMini",$tmpObj->dateBegin);}
			if(!empty($tmpObj->dateEnd))		{$tmpObj->pluginTooltip.="<hr>".Txt::trad("end")." : ".Txt::dateLabel("dateMini",$tmpObj->dateEnd);}
