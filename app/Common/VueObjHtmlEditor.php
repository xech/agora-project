...
$(function(){
	tinymce.init({
		...
		setup: function(editor){
			////	Met le focus dans l'éditeur (sauf si ya deja un focus OU en responsive, pour pas afficher le clavier virtuel)
			...
			if($(".attachedFileInput").exist()){
				editor.ui.registry.addButton("addMediaFileButton",{
					...
					onAction:function(_){
						$("[for='objMenuAttachedFile']").click();																//Affiche le menu des fichiers joints (cf. "VueObjAttachedFile.php")
						...
						fileInput.click().change(function(){  attachedFileInsert(this.id.replace('attachedFileInput',''));  });	//Récupere un fichier (trigger "click"+"change")  &&  Affiche le fichier dans l'editeur (ex d'Id: "attachedFileInput55" => 55)
					}
				});
			}
		},
...
function editorContent()
{
	...
	if(/attachedFileTagTmp/i.test(content)==false)  {return content;}//Renvoie le contenu s'il n'ya pas d'image/fichier temporaire (format "Blob" trop lourd pour "ap_userLivecouter.editorDraft")
}
...
/****************************************************************************************************************************************************
 *	FICHIER JOINT : REMPLACE LE "SRC" DES IMAGES TEMPORAIRES (FORMAT "BLOB") PAR UN ID TEMPORAIRE (cf. "VueObjMenuEdit.php" & "ModMail/index.php") 
 ****************************************************************************************************************************************************/
